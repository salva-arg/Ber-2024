...
bg.innerAtomID = this.parseIntStr(this.atts.get("inneratomid"));
bg.bondID = this.parseIntStr(this.atts.get("bondid"));
}return;
}if ("bracketedgroup".equals(localName)) {
var usage = this.atts.get("bracketusage");
if (this.bracketedGroups == null) this.bracketedGroups =  new java.util.Stack();
var ids = null;
var repeatCount = 0;
if ("MultipleGroup".equals(usage)) {
var sids = JU.PT.getTokens(this.atts.get("bracketedobjectids"));
ids =  Clazz.newIntArray (/*org.eclipse.jdt.core.dom.QualifiedName*/sids.length, 0);
for (var i = ids.length; --i >= 0; ) ids[i] = this.parseIntStr(sids[i]);

repeatCount = this.parseIntStr(this.atts.get("repeatcount"));
}this.bracketedGroups.add( new J.adapter.readers.xml.XmlCdxReader.BracketedGroup(ids, repeatCount));
}}, "~S,~S");
...
this.innerAtomID = 0;
this.repeatCount = 0;
Clazz.instantialize(this, arguments);}, J.adapter.readers.xml.XmlCdxReader, "BracketedGroup", null);
Clazz.makeConstructor(c$, 
function(ids, repeatCount){
this.ids = ids;
